...
UPDATE: should maybe take a linkedlist (see pair lambda) of funcs instead of 2 more curried funcs, so funcs can be constraint on any (usually small) set of funcs that are also in a "valid set", and the contents of such a linkedlist can be any randomly chosen funcs in that set. Vararg constraints are more flexible. But infinite number of possible params is even more flexible, 1 at a time, so (x true) would still be a place to put stuff and instead of (x false y z) its (x false false anyOtherFuncInValidSet false false anotherFuncInSet false false aFunc false true) and if that halts (ever for any combo of such sequence of inputs in a valid set) the constraint is disproven. (x false true) halts if x knows about a contradiction. (x false false y) is still mostly x but may have added y to a linkedlist to remember these things to check constraints on... So (x false false y true) is like (x true) but may actually be any func. Or something like that, cuz some things cant easily be done with just 3 at a time.
