...
UPDATE: should maybe take a linkedlist (see pair lambda) of funcs instead of 2 more curried funcs, so funcs can be constraint on any (usually small) set of funcs that are also in a "valid set", and the contents of such a linkedlist can be any randomly chosen funcs in that set. Vararg constraints are more flexible. But infinite number of possible params is even more flexible, 1 at a time, so (x true) would still be a place to put stuff and instead of (x false y z) its (x false anyOtherFuncInValidSet false anotherFuncInSet false aFunc true) and if that halts (ever for any combo of such sequence of inputs in a valid set) the constraint is disproven. Or something like that, cuz some things cant easily be done with just 3 at a time.
