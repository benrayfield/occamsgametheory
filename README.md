...
In my claims about functions in general, its sometimes important to understand that some lambda functions automatically have a pointer to themself for recursion (specifically in occamsfuncer opcodes numbered 1-7), which is not just a universal lambda function but also a pattern calculus function and quine. In fact, its impossible to create any occamsfuncer halted function which is not a quine aka (L x (R x))->x (and that halts trivially in constant time) for all halted x.
